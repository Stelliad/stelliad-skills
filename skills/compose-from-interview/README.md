...
```bash
cp -r compose-from-interview /path/to/your-project/.claude/skills/
```
...
compose-from-interview is for new content where the raw material does not exist yet.
...
- **`compose-score`** in this repository: scores the draft this produces against a stated bar. Give it the source file as well as the draft and it can check every specific against what you actually said, and it treats every `[NEED: ...]` marker as a question for you rather than something to write for you
