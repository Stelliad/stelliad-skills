...
`compose-from-interview` reads and writes the same file under the same key. There is one
learning loop across the pair, not one per skill: `compose-from-interview` applies the lessons
when drafting, `compose-score` records them after approval.
...
Two optional inputs change what this skill can see. Both come from `compose-from-interview` in
...
Neither is required. compose-score scores a draft from anywhere.
