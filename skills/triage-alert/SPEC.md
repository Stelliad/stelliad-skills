...
Automate production alert triage by ingesting monitoring alerts, performing root-cause diagnosis, and routing to the appropriate response (file, fix, escalate, suppress). Prioritizes investigation over immediate code changes.
...
```bash
triage-alert/scripts/sentry-fetch.sh PROJ-4F            # short id
triage-alert/scripts/sentry-fetch.sh <issue-url>        # full URL
