...
# ─────────────────────────────────────────────────────────────────
# triage-alert step 1: pull a Sentry issue + its latest event.
#
...
#   triage-alert/scripts/sentry-fetch.sh https://your-org.sentry.io/issues/1234567890/
#   triage-alert/scripts/sentry-fetch.sh 1234567890 --raw
#
# Env:
...
for dep in curl jq; do
  command -v "$dep" >/dev/null 2>&1 || { echo "triage-alert: $dep is required" >&2; exit 69; }
done

: "${SENTRY_AUTH_TOKEN:?triage-alert: SENTRY_AUTH_TOKEN is not set}"
...
api() {
  ...
  body="$(sed '$d' <<<"$body")"
  if [[ "$status" != 2* ]]; then
    echo "triage-alert: Sentry API ${1} returned HTTP ${status}" >&2
    echo "$body" >&2
    exit 70
  fi
  printf '%s' "$body"
...
if [[ "$TARGET" =~ ^https?:// ]]; then
  ...
  if [ -z "$ISSUE_ID" ]; then
    echo "triage-alert: could not find an issue id in ${TARGET}" >&2
    exit 65
  fi
  ...
  : "${SENTRY_ORG:?triage-alert: SENTRY_ORG is required to resolve a short id}"
  SHORT_UPPER="$(tr '[:lower:]' '[:upper:]' <<<"$TARGET")"
  ISSUE_ID="$(api "/organizations/${SENTRY_ORG}/shortids/${SHORT_UPPER}/" | jq -r '.groupId // empty')"
  if [ -z "$ISSUE_ID" ]; then
    echo "triage-alert: short id ${SHORT_UPPER} did not resolve in org ${SENTRY_ORG}" >&2
    exit 65
  fi
fi
