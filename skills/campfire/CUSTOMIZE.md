...
`gauntlet` reads `lessons_file` under the same key and writes to it after
approval. There is one learning loop across the pair, not one per skill.

**`source` is what makes the pair a pair.** When `gauntlet` scores a draft and
...
**`gap_marker` is shared too.** `campfire` writes it where the interview did not
produce something the draft needs. `gauntlet` treats any visible marker as an
...
4. **What is the review path after the draft?** `gauntlet` in this repository scores against a bar. If you use something else, say so in the draft stage so the handoff is explicit.
